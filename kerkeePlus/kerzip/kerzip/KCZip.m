...
@protocol KCZipDelegate <NSObject>

@optional

- (void)willUnzipAtPath:(NSString *)path zipInfo:(unz_global_info)zipInfo;
- (void)didUnzipAtPath:(NSString *)path zipInfo:(unz_global_info)zipInfo toPath:(NSString *)unzippedPath;

- (BOOL)shouldUnzipFileAtIndex:(NSInteger)fileIndex totalFiles:(NSInteger)totalFiles zipPath:(NSString *)zipPath fileInfo:(unz_file_info)fileInfo;
- (void)willUnzipFileAtIndex:(NSInteger)fileIndex totalFiles:(NSInteger)totalFiles zipPath:(NSString *)zipPath fileInfo:(unz_file_info)fileInfo;
- (void)didUnzipFileAtIndex:(NSInteger)fileIndex totalFiles:(NSInteger)totalFiles zipPath:(NSString *)zipPath fileInfo:(unz_file_info)fileInfo;
- (void)didUnzipFileAtIndex:(NSInteger)fileIndex totalFiles:(NSInteger)totalFiles zipPath:(NSString *)zipPath unzipPath:(NSString *)unzipPath;

- (void)onZipProgress:(unsigned long long)loaded total:(unsigned long long)total;
@end
...
+ (BOOL)unzip:(NSString *)path to:(NSString *)destination
{
    return [self unzip:path to:destination delegate:nil];
}

+ (BOOL)unzip:(NSString *)path to:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error
{
...
+ (BOOL)unzip:(NSString *)path to:(NSString *)destination delegate:(id<KCZipDelegate>)delegate
{
...
+ (BOOL)unzip:(NSString *)path to:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<KCZipDelegate>)delegate
{
...
+ (BOOL)unzip:(NSString *)path
        ...
        progressBlock:(void (^)(NSString *entry, unz_file_info zipInfo, long entryNumber, long total))progressHandler
      completionBlock:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
...
+ (BOOL)unzipFileAtPath:(NSString *)path
          toDestination:(NSString *)destination
              overwrite:(BOOL)overwrite
               password:(NSString *)password
                  error:(NSError **)error
               delegate:(id<KCZipDelegate>)delegate
      ...
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:@"KCZipArchiveErrorDomain" code:-1 userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (unzGoToFirstFile(zip) != UNZ_OK)
    {
        ...
        NSError *err = [NSError errorWithDomain:@"KCZipArchiveErrorDomain" code:-2 userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    // Message delegate
    if ([delegate respondsToSelector:@selector(willUnzipAtPath:zipInfo:)]) {
        [delegate willUnzipAtPath:path zipInfo:globalInfo];
    }
    if ([delegate respondsToSelector:@selector(onZipProgress:total:)]) {
        [delegate onZipProgress:currentPosition total:fileSize];
    }
    ...
    do {
        @autoreleasepool {
            ...
            // Message delegate
            if ([delegate respondsToSelector:@selector(shouldUnzipFileAtIndex:totalFiles:zipPath:fileInfo:)]) {
                if (![delegate shouldUnzipFileAtIndex:currentFileNumber
                                                     totalFiles:(NSInteger)globalInfo.number_entry
                                                    zipPath:path fileInfo:fileInfo]) {
                    success = NO;
                    canceled = YES;
                    break;
                }
            }
            if ([delegate respondsToSelector:@selector(willUnzipFileAtIndex:totalFiles:zipPath:fileInfo:)]) {
                [delegate willUnzipFileAtIndex:currentFileNumber totalFiles:(NSInteger)globalInfo.number_entry
                                             zipPath:path fileInfo:fileInfo];
            }
            if ([delegate respondsToSelector:@selector(onZipProgress:total:)]) {
                [delegate onZipProgress:(NSInteger)currentPosition total:(NSInteger)fileSize];
            }
            ...
            if (nil != err) {
                NSLog(@"[KCZipArchive] Error: %@", err.localizedDescription);
            }
            ...
            if (!fileIsSymbolicLink) {
                ...
                if (fp) {
                    if ([[[fullPath pathExtension] lowercaseString] isEqualToString:@"zip"]) {
                        ...
                        if ([self unzip:fullPath to:[fullPath stringByDeletingLastPathComponent] overwrite:overwrite password:password error:nil delegate:nil]) {
                    ...
                    if (fileInfo.dosDate != 0) {
                        ...
                        if (attr) {
                            if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
                                ...
                                NSLog(@"[KCZipArchive] Failed to set attributes - whilst setting modification date");
                            }
                        }
                    }
                    ...
                    if (permissions != 0) {
                        ...
                        if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            NSLog(@"[KCZipArchive] Failed to set attributes - whilst setting permissions");
                        }
            ...
            // Message delegate
            if ([delegate respondsToSelector:@selector(didUnzipFileAtIndex:totalFiles:zipPath:fileInfo:)]) {
                [delegate didUnzipFileAtIndex:currentFileNumber totalFiles:(NSInteger)globalInfo.number_entry
                                            zipPath:path fileInfo:fileInfo];
            } else if ([delegate respondsToSelector: @selector(didUnzipFileAtIndex:totalFiles:zipPath:unzipPath:)]) {
                [delegate didUnzipFileAtIndex: currentFileNumber totalFiles: (NSInteger)globalInfo.number_entry
                                            zipPath:path unzipPath: fullPath];
            }
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: d[@"modDate"]} ofItemAtPath:d[@"path"] error:&err]) {
            NSLog(@"[KCZipArchive] Set attributes failed for directory: %@.", d[@"path"]);
        }
        if (err) {
            NSLog(@"[KCZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
    ...
    // Message delegate
    if (success && [delegate respondsToSelector:@selector(didUnzipAtPath:zipInfo:toPath:)]) {
        [delegate didUnzipAtPath:path zipInfo:globalInfo toPath:destination];
    }
    // final progress event = 100%
    if (!canceled && [delegate respondsToSelector:@selector(onZipProgress:total:)]) {
        [delegate onZipProgress:fileSize total:fileSize];
    }
    ...
    if (crc_ret == UNZ_CRCERROR)
    {
        ...
        retErr = [NSError errorWithDomain:@"KCZipArchiveErrorDomain" code:-3 userInfo:userInfo];
    }
    if (error)
    {
        *error = retErr;
...
{
    return [KCZip createZip:path withPaths:paths withPassword:nil];
}
+ (BOOL)createZip:(NSString *)path withDir:(NSString *)directoryPath{
    return [KCZip createZip:path withDir:directoryPath withPassword:nil];
}

+ (BOOL)createZip:(NSString *)path withDir:(NSString *)directoryPath keepParentDir:(BOOL)keepParentDirectory{
    return [KCZip createZip:path withDir:directoryPath keepParentDir:keepParentDirectory withPassword:nil];
}

+ (BOOL)createZip:(NSString *)path withPaths:(NSArray *)paths withPassword:(NSString *)password
{
    BOOL success = NO;
    KCZip *zipArchive = [[KCZip alloc] initWithPath:path];
    if ([zipArchive open]) {
...
+ (BOOL)createZip:(NSString *)path withDir:(NSString *)directoryPath  withPassword:(NSString *)password{
    return [self createZip:path withDir:directoryPath keepParentDir:NO withPassword:password];
}
...
- (BOOL)close
{
    NSAssert((_zip != NULL), @"[KCZipArchive] Attempting to close an archive which was never opened");
    zipClose(_zip, NULL);
    return YES;
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime
{
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[KCZipArchive] MSDOS date masks don't add up");
